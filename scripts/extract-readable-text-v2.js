...
const OUTPUT_FILE = path.join(__dirname, 'NASB1995_cleaned_v9.txt');
const LOG_FILE = path.join(__dirname, 'extract-log-v2.txt');
...
function cleanText(text) {
  // First, replace any non-ASCII characters with spaces
...
async function processFile() {
  try {
    log('Starting to extract readable text from NASB1995.txt (v2)...');
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (verseMatch) {
        ...
        verseBuffer.push(cleanLine.trim());
      } else {
