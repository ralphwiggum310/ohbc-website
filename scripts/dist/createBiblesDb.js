...
function createEmptyTableLike(sourceDb, targetDb, sourceTable, targetTable) {
    ...
    const newCreateTableSql = createTableSql.replace(new RegExp(`\\b${sourceTable}\\b`, 'g'), targetTable);
...
createEmptyTableLike(sourceDb, targetDb, 't_king_james_bible', 't_new_international_version');
createEmptyTableLike(sourceDb, targetDb, 't_king_james_bible', 't_new_american_standard_bible');
...
const insertTranslation = targetDb.prepare('INSERT INTO Translation (id, name, table_name, abbreviation) VALUES (?, ?, ?, ?)');
