...
async function createTable(db) {
  ...
  console.log('t_nasb1995 table is ready for import.');
}
...
async function importVerses(db) {
  ...
  try {
    for (let i = 0; i < lines.length; i++) {
      ...
      // Skip chapter titles and other non-verse lines that don't start with a number
      if (!/^\d+\s+/.test(line)) {
        continue;
      }
      ...
      if (verseMatch && currentBook && currentChapter > 0) {
        ...
        if (verseText && verseText.length > 0) {
          try {
            ...
            console.error(`\nError inserting verse ${bookId}:${currentChapter}:${verseNum}:`, error.message);
            skippedLines++;
          }
        } else {
          skippedLines++;
    ...
    if (skippedLines > 0) {
      console.log(`Warning: ${skippedLines} lines were skipped during import.`);
    }
...
async function main() {
  let db;
  try {
    console.log('Starting NASB1995 import (v2)...');
    ...
    // Import the verses
    const verseCount = await importVerses(db);
    
    // Verify the import
    if (verseCount > 0) {
      const result = await db.get('SELECT COUNT(*) as count FROM t_nasb1995');
      console.log(`\nVerification: Database now contains ${result.count} verses.`);
    }
