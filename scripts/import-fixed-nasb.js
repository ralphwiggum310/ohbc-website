...
// File paths
const fixedFilePath = path.join(__dirname, 'NASB1995-fixed.txt');
...
async function importFixedNASB() {
  console.log('Starting import of fixed NASB1995 text...');
  ...
  try {
    ...
    // Read the fixed file
    console.log('Reading fixed text file...');
    const content = await readFile(fixedFilePath, 'utf-8');
    ...
    const insertMany = db.transaction((verses) => {
      for (const verse of verses) {
        try {
          ...
          console.error(`Error inserting verse:`, error.message);
        }
      }
    });
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Process verse line (format: "BOOK CHAPTER:VERSE text...")
      const verseMatch = line.match(/^(\w+)\s+(\d+):(\d+)\s+(.*)/);
      if (verseMatch) {
        ...
        if (!bookId) {
          console.warn(`  Unknown book in verse: ${bookName}`);
          continue;
        }
...
// Run the import
importFixedNASB().catch(console.error);
