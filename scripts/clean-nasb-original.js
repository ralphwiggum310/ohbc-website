...
async function cleanNASB() {
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Check for book name with more flexible matching
      if (isBookName(line)) {
        // Try to find the book name in the line
        let foundBook = null;
        
        // Try exact match first
        foundBook = bookNames.find(book => 
          line.trim().toUpperCase() === book
        );
        ...
        if (!foundBook) {
          foundBook = bookNames.find(book => {
            ...
            // Match at start of line
            if (lineUpper.startsWith(bookUpper) && 
                (lineUpper.length === bookUpper.length || /[\s\W]/.test(lineUpper[bookUpper.length]))) {
              return true;
            }
