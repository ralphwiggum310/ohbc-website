...
const bookNumberToKeyEnglishId = {
    ...
    710: 65,  // 3 John
    720: 66,  // Jude
    730: 66   // Revelation (mapped to same as Jude as per key_english table)
};
...
function updateNiv() {
    ...
    try {
        ...
        try {
            ...
            // Check if last_updated column exists in Translation table
            log('Checking for last_updated column in Translation table...');
            const hasLastUpdated = targetDb.prepare(`
              SELECT COUNT(*) as count 
              FROM pragma_table_info('Translation') 
              WHERE name = 'last_updated'`).get().count > 0;

            if (!hasLastUpdated) {
              log('Adding last_updated column to Translation table...');
              targetDb.prepare(`
                ALTER TABLE Translation 
                ADD COLUMN last_updated TEXT`).run();
            }

            // Update version information in the Translation table
            log('Updating version information...');
            const updateVersionStmt = targetDb.prepare(`
              UPDATE Translation 
              SET last_updated = ? 
              WHERE id = 'niv'`);
            updateVersionStmt.run(new Date().toISOString());
