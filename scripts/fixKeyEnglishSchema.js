...
const BIBLE_BOOKS = [
  ...
  // ... (rest of the books remain the same as in populateKeyEnglish.js)
...
async function fixKeyEnglishSchema() {
  const dbPath = path.join(process.cwd(), 'Bible api', 'C:\WindSurf\ohbc_website\data\bible\bibles.db');
  console.log(`Fixing key_english table in: ${dbPath}`);
  
  try {
    const db = sqlite3(dbPath);
    
    // Check if key_english table exists and get its structure
    const tableInfo = db.prepare("PRAGMA table_info(key_english)").all();
    console.log('Current key_english table structure:');
    console.table(tableInfo);
    
    // Check if we need to recreate the table
    const needsRecreate = tableInfo.length === 0 || 
                         !tableInfo.some(col => col.name === 'abbreviation') ||
                         !tableInfo.some(col => col.name === 'chapters');
    
    if (needsRecreate) {
      console.log('Recreating key_english table with correct schema...');
      
      // Drop the existing table if it exists
      db.prepare('DROP TABLE IF EXISTS key_english').run();
      ...
      console.log('Created new key_english table with correct schema');
    } else {
      console.log('key_english table already has the correct schema');
    }
    
    // Check if the table is empty
    const rowCount = db.prepare('SELECT COUNT(*) as count FROM key_english').get().count;
    
    if (rowCount === 0) {
      console.log('Populating key_english table with Bible books...');
      ...
      insertMany(BIBLE_BOOKS);
      
      console.log(`Inserted ${BIBLE_BOOKS.length} books into key_english`);
    } else {
      console.log(`key_english table already contains ${rowCount} rows`);
    }
    
    // Verify the data
    const books = db.prepare('SELECT * FROM key_english ORDER BY id').all();
    console.log('First 5 books:');
    console.table(books.slice(0, 5));
    console.log('Last 5 books:');
    console.table(books.slice(-5));
    
    db.close();
    console.log('Successfully fixed key_english table');
    
  } catch (error) {
    console.error('Error fixing key_english table:', error);
    process.exit(1);
  }
}
