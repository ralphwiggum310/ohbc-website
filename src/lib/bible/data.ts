...
// Helper function to get a book by name
export function getBookByName(name: string): BibleBook | undefined {
  return BIBLE_BOOKS.find(book => book.name === name);
}

// Helper function to get all books in a testament
export function getBooksByTestament(testament: 'old' | 'new'): BibleBook[] {
  return BIBLE_BOOKS.filter(book => book.testament === testament);
}

// Helper function to get the next/previous book
export function getAdjacentBook(currentBookName: string, direction: 'next' | 'previous'): BibleBook | null {
  const currentIndex = BIBLE_BOOKS.findIndex(book => book.name === currentBookName);
  if (currentIndex === -1) return null;
  
  const newIndex = direction === 'next' ? currentIndex + 1 : currentIndex - 1;
  return BIBLE_BOOKS[newIndex] || null;
}
